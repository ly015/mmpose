...
class Transform(metaclass=ABCMeta):
    ...
    def __init__(self, key_mapping: Optional[dict] = None):
    ...
    def _static_check_input_mapping(self, key_mapping):

        input_mapping = key_mapping.get('input', None)
    ...
    def _dynamic_check_input_mapping(self, key_mapping, result):
        input_mapping = key_mapping['input']
    ...
    def _dynamic_check_output_key_mapping(self, key_mapping, output):
        output_mapping = key_mapping.get('output', None)
    ...
    def collect_input(results: dict,
        ...
        def _collect(results, input_mapping):
            ...
            if isinstance(input_mapping, (tuple, list)):
                return input_mapping.__class__(
                    (_collect(results, e) for e in input_mapping))
            try:
        ...
        return _collect(results, input_mapping)
...
class Resize:
    """Resize images & bbox & mask. This transform resizes the input image to
    some scale. Bboxes and masks are then resized with the same scale factor.
    If the input dict contains the key "scale", then the scale in the input
    dict is used, otherwise the specified scale in the init method is used. If
    the input dict contains the key "scale_factor" (if MultiScaleFlipAug does
    not give img_scale but scale_factor), the actual scale will be computed by
    image shape and scale_factor. `img_scale` can either be a tuple (single-
    scale) or a list of tuple (multi-scale). There are 3 multiscale modes:
